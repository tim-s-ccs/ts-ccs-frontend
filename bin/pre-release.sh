...
if [[ -n $(git status --porcelain) ]]; then
  ...
  git commit -m "Release CCC Components to '$BRANCH_NAME' for testing"
...
echo
echo "✅ Success! To install the pushed branch release, run 'npm install --save tim-s-ccs/tim-ccs-components#$BRANCH_COMMIT_SHA'"
