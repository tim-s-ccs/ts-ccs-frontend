# Documation

This file provides links for the documentation.

## Instillation

[Install using precompiled files](/docs/instillation/install-using-compiled-files.md)

[Install with Node.js package manager (npm)](/docs/instillation/install-with-npm.md)

## Guides

[Get started](/docs/guides/get-started.md)

[Using CCS Frontend with GOV.UK Prototype Kit](/docs/guides/govuk-prototype-kit.md)

[Import CSS, assets and JavaScript](/docs/guides/import-assets.md)

[Update using Node.js package manager (npm)](/docs/guides/update-with-npm.md)

[Use Nunjucks](/docs/guides/use-nunjucks.md)

## Contributing

[Application architecture](/docs/contributing/application-architecture.md)

[Changelog template](/docs/contributing/CHANGELOG_TEMPLATE.md)

[Polyfilling](/docs/contributing/polyfilling.md)

[Resize text in browsers](/docs/contributing/resize-text-in-browsers.md)

[Running locally](/docs/contributing/running-locally.md)

[NPM and Gulp tasks](/docs/contributing/tasks.md)

[Test components using accessibility acceptance criteria](/docs/contributing/test-components-using-accessibility-acceptance-criteria.md)

[Test your CCS Frontend contribution](/docs/contributing/testing.md)

[Versioning](/docs/contributing/versioning.md)

### Cosing Standards

[Components](/docs/contributing/coding-standards/components.md)

[CSS Style Guide](/docs/contributing/coding-standards/css.md)

[JavaScript style guide](/docs/contributing/coding-standards/js.md)

[Component API definition and use](/docs/contributing/coding-standards/nunjucks-api.md)

## Releaseing

[Publish pre-release of CCS Frontend](/docs/releasing/publishing-a-pre-release.md)

[Publishing from a support branch](/docs/releasing/publishing-from-a-support-branch.md)

[Publishing](/docs/releasing/publishing.md)

[Testing and linting](/docs/releasing/testing-and-linting.md)
