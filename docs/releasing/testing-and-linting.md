...
We test individual components and for instance global Sass files. See [all.test.js](../../src/ccs/all.test.js). for examples of global tests we run, and  [header.test.js](../../src/ccs/components/header/header.test.js) for an example of component tests.
...
You can run a subset of the test suite that only tests components by running:
    ...
    `npm test -- src/ccs/components/footer`

Note: There's a watch mode that keeps a testing session open waiting for changes that can be used with:

    `npm test -- --watch src/ccs/components/footer`
...
`npm test -- -u src/ccs/components/header`
